...
async def get_project_info(request: Request, project_id: str):
    ...
    return JSONResponse(
        content={
            "signal": ResponseSignal.VECOTRDB_COLLECTION_RETREIVED.value,
...
async def search_index(request: Request, project_id: str, search_request: SearchRequest):
    ...
    if not results:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "signal": ResponseSignal.VECOTRDB_SEARCH_ERROR.value
            }
        )
    ...
    return JSONResponse(
        content={
            "signal": ResponseSignal.VECOTRDB_SEARCH_SUCCESS.value,
